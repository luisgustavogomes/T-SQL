...
-- Abrir app 
-- D:\Fabiano\Trabalho\Sr.Nimbus\Cursos\SQL26 - SQL Server - Mastering the database engine (former Internals)\Slides\Módulo 02 - Memória parte 1\6 - Memory clerks - CachePlan\Cache Plans Test
/*
...
-- Caso não seja possível corrigir a app, considerar TF 144 -- http://blogs.msdn.microsoft.com/sqlprogrammability/2007/01/13/6-0-best-programming-practices
...
-- Considerando o comentário abaixo... Posso ter 16k desses lixos em cache... 
...
-- Abrir app
-- D:\Fabiano\Trabalho\Sr.Nimbus\Cursos\SQL26 - SQL Server - Mastering the database engine (former Internals)\Slides\Módulo 02 - Memória parte 1\6 - Memory clerks - CachePlan\sp_prepare vs direct exec
...
-- Talvez em um ambiente com baixa latência de rede, o sp_prepare ajude, já que na teoria irá reduzir o tamanho do pacote enviado pela rede... Azure com serv no Japão...?...
...
---- mesmo usuário e user seetings utilizados na app


/*

  Demo 10 - Como o cacheplan responde a uma pressão de memória? 
...
-- Reiniciar a instância pra ter uma visão zerada da sys.dm_os_memory_cache_clock_hands
...
-- dm_os_memory_cache_clock_hands não removeu nada...
...
-- Se necessario criar a sp... ...Módulo 02 - Memória parte 1\6 - Memory clerks - CachePlan\st_QueryCostAlto.sql
...
-- dm_os_memory_cache_clock_hands passou facão e removeu vários planos...
SELECT dm_os_memory_cache_clock_hands.*, DATEADD(ms, last_tick_time - info.ms_ticks, GETDATE()) FROM sys.dm_os_memory_cache_clock_hands
CROSS JOIN sys.dm_os_sys_info AS info
WHERE [type] = 'CACHESTORE_SQLCP' AND name = 'SQL Plans'
GO

-- Será que o plano com custo alto morreu?
SELECT Ecp.usecounts, dm_exec_sql_text.text, dm_os_memory_cache_entries.*
FROM sys.dm_exec_cached_plans as ECP
CROSS APPLY sys.dm_exec_sql_text(ECP.plan_handle)
INNER JOIN sys.dm_os_memory_cache_entries
...
-- Eu acho que deveria ter deixado ele lá... mass... pressão é pressão né pai... 
-- Quem tem juizo obedece... clerk liberou a memória, o máximo que pode...
...
-- Cursores são armazenados em memória... mais precisamente, no cache plan...

-- Abrir nova sessão e rodar o seguinte código:
...
-- Fechar sessão que criou os cursores...
